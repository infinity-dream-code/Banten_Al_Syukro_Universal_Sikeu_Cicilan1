(function () {
    ...
    function requestMethod(input, init, settings) {
        if (settings && (settings.type || settings.method)) {
            return String(settings.type || settings.method).toUpperCase();
        }
        if (init && init.method) {
    ...
    function shouldRetryStatus(status, method) {
        ...
        return status === 500 && (!method || String(method).toUpperCase() === 'GET');
    }
    ...
    window.fetch = function (input, init) {
        ...
        return nativeFetch(firstInput, firstInit).then(function (res) {
            if (!shouldRetryStatus(res.status, requestMethod(input, init))) {
                return res;
            }
    ...
    function patchJQueryAjax($) {
        ...
        $.ajax = function (url, settings) {
            ...
            first.done(function () {
                ...
                if (!jqXHR || !shouldRetryStatus(jqXHR.status, requestMethod(null, null, settings))) {
                    fail(this, arguments);
                    return;
                }
